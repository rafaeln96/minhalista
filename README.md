...
- **PWA Completo & Engenharia Avançada para iOS Safari**: Instalação direta ("Adicionar à Tela de Início"). Resolvidos bugs nativos do motor WebKit (Safari), utilizando alturas dinâmicas modernas (`min-height: 100dvh`) para evitar rolagem falsa provocada pelas barras de navegação do celular, e transições CSS (`cubic-bezier`) acopladas ao invés de forçar recriações no DOM, erradicando os *scroll jumps* causados pelo fechamento do teclado nativo no iOS.
